...
function getFaceBoxes(results, img_width, img_height) {
  ...
  let face_boxes = [];
    ...
    face_boxes.push([xmin, xmax, ymin, ymax, prob]);
    // console.log([xmin,ymin,xmax-xmin, ymax-ymin]);
    // drawFaceBoxes(ctx, xmin, xmax, ymin, ymax, prob);
  }
  return face_boxes;
...
function drawFaceBoxes(image, canvas, face_boxes) {
  ...
  ctx.drawImage(image, 0, 0, image.videoWidth, image.videoHeight);
  }
...
function drawKeyPoints(canvas, Keypoints, boxes) {
  ...
  for (let i = 0; i < 128; i = i + 2) {
    true_keyPoints[i] = (box[1] - box[0]) * keypoints[i] + box[0];
    true_keyPoints[i + 1] = (box[3] - box[2]) * keypoints[i + 1] + box[2];
  }
  for (let i = 0; i < 128; i = i + 2) {
    ctx.beginPath();
    ctx.fillStyle = "blue";
    ctx.strokeStyle = "blue";
    ctx.arc(true_keyPoints[i], true_keyPoints[i + 1], 2, 0, 2 * Math.PI);
    ctx.fill();
    ctx.closePath();
  }
});
